...
namespace InsuranceManagement.Web.Infrastructure;

public class AppDataStore
{
    private readonly object _lock = new();
    private int _leadId = 200;
    private int _accountId = 200;
    private int _activityId = 200;
    private int _saleId = 200;
    private int _expenseId = 200;
    private int _importId = 10;
    ...
    public List<ImportBatch> ImportBatches { get; } = [];
    ...
    public int NextLeadId()
    {
        lock (_lock) { return ++_leadId; }
    }

    public int NextAccountId()
    {
        lock (_lock) { return ++_accountId; }
    }

    public int NextActivityId()
    {
        lock (_lock) { return ++_activityId; }
    }

    public int NextSaleId()
    {
        lock (_lock) { return ++_saleId; }
    }

    public int NextExpenseId()
    {
        lock (_lock) { return ++_expenseId; }
    }

    public int NextImportId()
    {
        lock (_lock) { return ++_importId; }
    }

    public string CreateLeadCode(int id) => $"LD-{id}";
    public string CreateAccountCode(int id) => $"ACC-{id}";
    public string CreateActivityCode(int id) => $"ACT-{id}";
    public string CreateSaleCode(int id) => $"SAL-{id}";
    public string CreateExpenseCode(int id) => $"EXP-{id}";

    public Employee? GetEmployee(int id) => Employees.FirstOrDefault(x => x.Id == id);
    public Account? GetAccount(int id) => Accounts.FirstOrDefault(x => x.Id == id);
    public Lead? GetLead(int id) => Leads.FirstOrDefault(x => x.Id == id);
    public Activity? GetActivity(int id) => Activities.FirstOrDefault(x => x.Id == id);
    public Sale? GetSale(int id) => Sales.FirstOrDefault(x => x.Id == id);
    public Expense? GetExpense(int id) => Expenses.FirstOrDefault(x => x.Id == id);
    public UserAccount? GetUser(int id) => Users.FirstOrDefault(x => x.Id == id);
    ...
    private void Seed()
    {
        ...
        Accounts.AddRange(
        [
            new Account { Id = 101, Code = "ACC-101", AccountType = AccountType.Corporate, DisplayName = "Acme Dis Ticaret A.S.", City = "Istanbul", District = "Besiktas", Phone = "0212 111 11 11", Email = "ik@acme.com", TaxNumber = "1234567890", OwnerEmployeeId = 1, Notes = "Kurumsal BES potansiyeli." },
            new Account { Id = 102, Code = "ACC-102", AccountType = AccountType.Corporate, DisplayName = "Nova Lojistik", City = "Kocaeli", District = "Izmit", Phone = "0262 222 22 22", Email = "info@nova.com", TaxNumber = "9876543210", OwnerEmployeeId = 3, Notes = "Hayat ve saglik urunleri icin acik firsat." },
            new Account { Id = 103, Code = "ACC-103", AccountType = AccountType.Individual, DisplayName = "Burcu Cetin", City = "Istanbul", District = "Kadikoy", Phone = "0532 555 11 22", Email = "burcu@example.com", OwnerEmployeeId = 2, Notes = "Bireysel hayat urunu ilgisi." }
        ]);
